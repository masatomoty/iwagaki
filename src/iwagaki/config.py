...
#:
#: **この軸はほぼ効かないことが実測で分かった**（2026-08、東舞鶴。旧「窪地の
#: 最低セル」配置では敷高が -0.30〜+0.30 m T.P. に収まり潮位スライダの実用域の
#: 外で、既往最高潮位 0.93 m の浸水面積は 3 ケースとも 24.85 ha で完全一致した）。
#: **`scripts/32` を越流点ベースに変えた後**（2026-09-01）は陸側端が鞍部に上がって
#: 敷高が 0.6〜1.1 m T.P. になり、東舞鶴の +0.20 m ケースだけ 0.93 m で 20.46 →
#: 20.18 ha（0.28 ha / 1.4 %）動く。西舞鶴は依然 3 ケース完全一致 [実測]。
#: 差は 1.4 % 以下なので結論は変わらない（`docs/results.md`
#: 「越流点ベースで 2 段が半分に減る」「敷高の仮定は結果をほぼ支配していない」）。
#:
