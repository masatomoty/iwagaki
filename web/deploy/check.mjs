...
// （docs/WEB_RESULTS.md §7）。ここで確認するのは主に次の 3 点:
//
...
// クロスオリジンになり transferSize が 0 になる（docs/WEB_DESIGN.md §8.3）
{
  const urls = []
...
// ---- 4. 圧縮とキャッシュ ----------------------------------------------------
{
  ...
  // 570 kB の geojson は first_meaningful_render の 2 割を占めうる（docs/WEB_DESIGN.md §5）。
