...
export function renderInspector(el: HTMLElement, store: Store, catalog: Catalog) {
  ...
  el.innerHTML = `
    ...
    <table>
      <tr><td></td><td class="num">PLATEAU 5m</td><td class="num">0.5m</td></tr>
      <tr><td>地盤高</td><td class="num">${fmt(a.groundElev.baseline)}</td><td class="num">${fmt(a.groundElev.highres)}</td></tr>
      <tr><td>h_conn</td><td class="num">${fmt(a.hConn.baseline)}</td><td class="num">${fmt(a.hConn.highres)}</td></tr>
      <tr><td>浸水深</td><td class="num">${fmt(db)}</td><td class="num">${fmt(dh)}</td></tr>
      ${isRoad ? `<tr><td>通行</td><td class="num">${ROAD_CLASS_LABEL[roadClass(db, th)]}</td>
                      <td class="num">${ROAD_CLASS_LABEL[roadClass(dh, th)]}</td></tr>` : ''}
      <tr><td>差 (0.5m − 5m)</td><td class="num" colspan="2">
        ...
        浸水深 ${fmt(dh - db)}</td></tr>
      ${band ? `<tr><td>判定が割れる水位帯</td><td class="num" colspan="2">
        ${band[0].toFixed(2)} 〜 ${band[1].toFixed(2)} m T.P.</td></tr>` : ''}
      ${a.areaM2 !== undefined ? `<tr><td>面積</td><td class="num" colspan="2">${a.areaM2.toFixed(1)} m²</td></tr>` : ''}
      ${a.sectionTypeLabel ? `<tr><td>区間種別</td><td class="num" colspan="2">${a.sectionTypeLabel}</td></tr>` : ''}
    </table>
