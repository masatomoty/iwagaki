...
export class ThreePointCloudRenderer implements PointCloudRenderer {
  ...
  /**
   * **セッタは値が変わったときだけ `onChange()` を出す。**
   * `onChange` は main.ts の `refresh()` を呼び、`refresh()` はここのセッタを
   * 無条件に呼ぶ。素直に毎回通知すると相互再帰で
   * `RangeError: Maximum call stack size exceeded` になる（?pc=1 で実測）。
   */
  setStyle(s: PointStyle) {
    if (this.style.pointSize === s.pointSize && this.style.opacity === s.opacity) {
      this.style = s
      return
    }
    this.style = s
