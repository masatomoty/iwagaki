...
export class Viewer {
  ...
  /**
   * zoom からカメラ距離を決める。zoom は「画面 1 px が何メートルか」の指標なので、
   * 距離はビューポート高に比例する。
   *
   * **タイル 1 枚 = 256 px 基準**（`getZoom()` と同じ規約で、往復する）。
   * MapLibre の `getZoom()` は 512 px 基準なので、**同じ景色でも値が 1 ずれる**
   * （256 基準 = MapLibre + 1）。旧実装の初期視点を再現するときは
   * `view/map.ts` の `INITIAL_ZOOM` を見ること。
   */
  setZoom(z: number) {
  ...
  /**
   * 画面のスケールを zoom 値で返す。**タイル 1 枚 = 256 px 基準**。
   *
   * この規約は偶然ではなく、`TerrainTiles` が要求するタイルの z がそのまま
   * `Math.round(getZoom())` になるように選んである。deck.gl TileLayer も
   * tileSize 256 で `MapLibre zoom + 1` の z を要求していたので、
   * **同じ視点なら同じ z のタイルを取る**（移行前後で比較が成立する）。
   */
  getZoom(): number {
  ...
  /** 注視点の経緯度 [lon, lat]。MapLibre の `map.getCenter()` 相当 */
  getCenterLngLat(): [number, number] {
  ...
  unproject(px: number, py: number): [number, number] | null {
    ...
    const ndc = new Vector3((px / w) * 2 - 1, -((py / h) * 2 - 1), -1)
  ...
  /**
   * 経緯度 -> 画面座標 [px]（地面 z=0 の点として）。MapLibre の `map.project()` 相当。
   * カメラの後ろに回った点は null。計測ハーネスが「この地点をクリックする」のに使う。
   */
  project(lon: number, lat: number): { x: number; y: number } | null {
    const [wx, wy] = lngLatToWorld(this.frame, lon, lat)
    const v = new Vector3(wx, wy, 0).project(this.camera)
    if (v.z > 1) return null
    const c = this.canvas
    return {
      x: ((v.x + 1) / 2) * (c.clientWidth || 1),
      y: ((1 - v.y) / 2) * (c.clientHeight || 1),
    }
  }

  /**
   * 経緯度・zoom で視点を決める。MapLibre の `map.jumpTo()` 相当で、
   * 計測ハーネス（`perf/*.mjs`）が視点を再現するのに使う。
   *
   * **zoom はタイル 1 枚 = 256 px 基準**（`setZoom` と同じ）。MapLibre 時代の
   * スクリプトが持っている zoom 値は 512 px 基準なので、渡すときに +1 する。
   */
  jumpTo(o: { center?: [number, number]; zoom?: number; pitch?: number; bearing?: number }) {
    const patch: Partial<CameraState> = {}
    if (o.center) patch.target = lngLatToWorld(this.frame, o.center[0], o.center[1])
    if (o.pitch !== undefined) patch.pitch = o.pitch
    if (o.bearing !== undefined) patch.bearing = o.bearing
    this.setCamera(patch)
