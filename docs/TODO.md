...
| C3 | **中** | **高潮浸水想定区域図と本リポジトリの結果を突き合わせる** | **舞鶴は高潮浸水想定区域に指定されていた**（`docs/DATA.md` §4。以前「指定は無い」と書いていたのは誤り）。公表は PDF 7 枚で GIS データの配布は確認できていない。まず吉原がどの図面に含まれるかを確認する。含まれていれば**外部の公的想定との比較**という、いままで無かった検証軸が 1 本増える |
