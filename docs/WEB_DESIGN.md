...
| 事象 | 対処 |
|---|---|
| **タイルの中で南北がひっくり返っていた。** 板（`BitmapLayer`）のときは deck.gl が `texCoords` に `1-v` を入れていたので起きなかった問題を、自前メッシュで持ち込んだ（`docs/WEB_RESULTS.md` §7.1） | `texUv()` で `1-v` してから引く。照合の道具は `web/perf/tileorient.mjs` |
...
計測パネルは**既定で非表示**。`?perf=1` かキーボードの `P` で開く。
`PerfRecorder` 自体は常に動いているので、パネルを閉じていても
`window.__iwagaki.snapshot()` は同じ値を返す（ハーネスは影響を受けない）。
内訳（coalesce 群数、GPU upload、draw call…）を読むのは開発者だけで、
浸水を見に来た人には意味を持たない。
