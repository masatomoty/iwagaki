...
**判断**: v1 は `setTerrain()` を**使わない**。地面は平面（z=0）とし、起伏は hillshade で表現する。
...
**[未確認]** maplibre-gl 6.x + deck.gl 9.3 の組み合わせでこの制約が緩和されているか。
v1 実装時に実機で確認し、結果をここに追記する。
...
`COALESCE_GAP` を振って「リクエスト数 × RTT」と「無駄バイト」の交点を実測で決める。
