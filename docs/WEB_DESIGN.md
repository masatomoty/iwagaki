...
- カメラプリセット（キーボード 1〜6）。キューブは面のクリックなので、
  ...
  **メニューに出すのは `横から` と `ホーム` の 2 つだけ。** `横から` は `west`
  （方位 90°・正射）で、`catalog.default_section` から出した汀線の方位 75° に最も近い。
  **`平面` はメニューに出さない**（上から見ても浸水域の輪郭しか分からず、
  それは既定の俯瞰でも読める）。平面・南↑・北↓・東← はキー 1 2 4 5 とキューブに残す。
...
| 種類 | 例 | 置き場所 |
|---|---|---|
| **変数** | 地形条件、水位 H、絞り込み、レイヤ、鉛直強調 | 操作パネル。**頭（地形条件と水位）は固定**し、それ以外だけスクロールする |
...
- **下位オプションは入れ子にする。** 建物の塗り分けは独立した節ではなく
  「PLATEAU 建物」の子
- **チェックボックスが嘘をつかないこと。** 鉛直強調中は PLATEAU 建物を隠すので、
  チェック自体を無効にして理由を出す
...
| 出力 | 実体 |
|---|---|
| 選択中の輪郭（白） | `SemanticsMesh` の専用 `LineSegments`。1 地物ぶんだけ作り直す |
| ホバー中の輪郭（水色） | 同上 |
| 周りの減光 | `aColor` を 0.42 倍。**隠しはしない** |

**輪郭は `depthTest: false` で描く。** 地物ポリゴンは地面の高さにあり、
...
彩度は 1.15 倍まで。**1.35 まで上げると SLAM のノイズ点（空に散る水色）が目立つ。**
明るさは触らない（実測色を作り変えない）。RGB を持たない配信物
（DTM から作った合成点群）に当たったときだけ紫の単一色相ランプに落ちる。
