...
点の色は decode worker が標高から作る（メインスレッドで回すと 300 万点でフリーズする）。
**紫の単一色相で、明るさだけを標高に振る。**

以前は 0〜12 m を青→緑→橙に振る虹だったが、2 つ壊れていた。

| 問題 | 中身 |
|---|---|
| 帯が潰れる | 吉原の市街は **T.P. 0〜3 m しか無い**ので、肝心の範囲がランプの下 1/4 に入る。12 m 以上は全部同じ色で、街の中はほぼ単色だった |
| 主張の色と衝突する | **青は浸水深、赤と黄は判定差、黄は点群の被覆線**が使っている。虹はそのどれとも重なるので、点群と判定の区別が付かない |

紫はどの主張も使っていない。**「これは観測であって判定ではない」が色だけで分かる。**
範囲は 0〜8 m で、`t^0.6` で低い側にランプを寄せる（街路の 0〜3 m に階調を回す）。
上端は白に寄せない。白にすると紫という手掛かりが消えて、陰影を付けた地形と
見分けが付かなくなる。
