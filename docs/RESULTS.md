# RESULTS — 第一段階 vertical slice の結果
...
> 成功条件: 実際の高解像度地形と粗い地形を同一高潮水位で比較し、
> 浸水判定が変わる地点を最低1つ、地物IDつきで確認できること。

水位 **H = 1.0 m T.P.** において、**905件の信頼可能なPLATEAU地物のうち 137件（15.1%）で浸水判定が変わった。**
条件は1つどころではなく、明確に成立している。
...
| 条件 | 地形 | 格子 |
|---|---|---|
| `baseline` | PLATEAU 地形モデル LOD1 TIN | 5.0 m |
| `highres` | 京都府 数値標高モデル 0.5 m（航空レーザ計測 2019-2023） | 0.5 m |
| `control` | `highres` を平均で 5 m に集約 | 5.0 m |

浸水計算は全条件で同一設定: 水位 0.0〜3.0 m T.P. / 刻み 0.05 m / 4近傍連結 /
seed（開放水面 297,968 m²）は高解像度地形から1度だけ求めて全条件で共有。
陸域 702,033 m²。
...
## 実点群を入れるとどうなるか（2026-08-22 受領・追記）
...
| 項目 | 実測値 |
|---|---|
| AOI（100 ha）に占める被覆 | **7.13 ha (7.1%)** — 歩いた線に沿った帯だけ |
| 被覆域の密度 | **5,478 点/m²**（0.5m セルあたり中央値 174 点） |
| 地面として使えたセル | **2.98 ha (3.0%)** — 壁面・軒下しか見ていないセルを除いた後 |
...
| **0.5m DEM vs 点群融合** | **0.35 %** | **236 / 1,576 m²** |
...
| 量 | 値 |
|---|---:|
| 護岸帯のうち点群が入っているセル | **0.207 ha**（帯全体のごく一部） |
| 点群 − DEM の中央値 | +0.020 m |
| p05 / p95 | **−0.600** / +0.219 m |
| \|差\| > 0.10 m のセル | **39.6 %** |
...
`docs/DESIGN.md` §8 の通り。特にこの結果に直接効くもの:

- **静水位モデル**。越流の時間発展・流量・波の打ち上げを含まない。
- **暗渠・樋門・排水施設を考慮していない。** 実際の連結性はこれらに強く依存する。
- 京都府DEMは**森林資源把握を目的**とした成果で、市街地の精度は保証されていない。
- 水位 H は**指定ハザードではないパラメータ**。舞鶴に高潮浸水想定区域の指定は存在しない
  （`docs/DATA.md` §4）。H.W.L. / 既往最高潮位 / 設計高潮位は **[未確認]**。
- **高解像度地形はまだ点群由来ではない**（京都府 0.5m DEM ラスタ）。
  点群提供後に `scripts/21_build_highres.py --source pointcloud` で差し替えて再評価する。
...
| 優先 | 内容 | 理由 |
|---|---|---|
| 高 | 点群からの DTM 生成に差し替え | 本来の目的。差の要因分解が「データソース優位」だったので、地形観測の質を上げる効果が大きい |
| 高 | 舞鶴の H.W.L. / 既往最高潮位 / 設計高潮位の確定 | 現在は水位が根拠のないパラメータ。目盛りが入るだけで結果の意味が変わる |
| 中 | 吉原入江の護岸天端高の把握 | 「越流する水位」を直接特定できる |
| 中 | 樋門・排水施設の位置を seed / 障壁に反映 | 連結性モデルの最大の弱点 |
| 中 | Webアプリ | 判定変化を地物単位で見せる |
| 低 | AOI の拡大 | まず吉原で手法を固めてから |
