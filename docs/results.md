...
| 条件 | 建物 浸水 / 694 | 浸水深 中央 / p90 |
|---|---:|---|
| PLATEAU 5m | **281** | 14 / 29 cm |
| 5m 対照 | 148 | 21 / 53 cm |
| 0.5m | **159** | 17 / 47 cm |
| 0.5m ＋ 点群 | 170 | 19 / 50 cm |

**この水位では PLATEAU 5m が最多になる（281）。** 換算前の `H = 0.68` では
...
1. **件数はモデルが多めに出る。** 0.5m 地形は 159 件だが、実測は AOI 内の 2 字で
...
3. **標高成果の換算で結論が変わった。** 換算前（`H = 0.68`）は
   ...
   換算後（`H = 0.870`）は PLATEAU 5m が 281 で最多になる。
...
| H [m T.P.] | 浸水域 | 浸水体積 | 建物 |
|---:|---:|---:|---:|
| 0.80 | 2.15 ha | 8,496 m³ | 78 |
| 0.85 | 2.15 ha | 9,569 m³ | **159** |
| **0.87**（イベント） | **3.98 ha** | 11,636 m³ | **159** |
| 0.90 | 9.15 ha | 15,102 m³ | 222 |
| 0.93（既往最高） | 9.15 ha | 17,848 m³ | 307 |
...
78 → 159 件に倍増する。**水位か天端高が 2 cm 違えば、浸水域の半分と建物の半分が消える。**
