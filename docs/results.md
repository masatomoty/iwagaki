...
| **最高潮位（気象庁 速報値）** | **T.P. +0.68 m** | **T.P. +0.67 m** |
| 床上浸水 | **0 戸** | **0 戸** |
| 床下浸水 | **39 戸** | **19 戸** |
| 道路冠水 | 33 箇所 | 27 箇所 |
| 浸水位 | 1〜20 cm | 3〜20 cm |
...
| 条件 | 建物 浸水 / 694 | 浸水深 中央 / p90 | 道路 / 217 |
|---|---:|---|---:|
| PLATEAU 5m | **29** | 9 / 22 cm | 8 |
| 5m 対照 | 68 | 26 / 45 cm | 12 |
| 0.5m | **60** | 22 / 38 cm | 9 |
| 0.5m ＋ 点群 | 62 | 26 / 38 cm | 10 |

`H = 0.68 m`（第 1 回の最高潮位）での判定。読み取れることと、読み取れないこと。

1. **件数はモデルが多めに出る。** 0.5m 地形は 60 件だが、実測は AOI 内の 2 字で
...
2. **浸水深はモデルの方が深い。** 実測の浸水位は 5・10・20 cm に集中するのに、
   0.5m 地形の分布は 20〜50 cm に過半（31/60 件）が乗る。
   **ただし「浸水位」の定義が未確認** [未確認] — 地盤面からの水深か、床面からの
   高さかで意味が変わる。**市に確認するまでこの比較は結論にしない。**
3. **PLATEAU 5m は少なすぎ（29）、5m 対照は多すぎる（68）。** 実測に近い側は
   0.5m（60）と点群融合（62）だが、1 の理由で件数だけでは決められない。
