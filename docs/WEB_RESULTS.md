...
**閾値の式がそもそも働いていない**ことが分かった。

予算は `maxBytes = clamp(bandwidthBps * 6, 2 MB, 20 MB)`。プロファイル別の実測:

| profile | 実効帯域 | `bandwidthBps` の推定 | `bw * 6` | 実際の予算 |
|---|---:|---:|---:|---|
| slow-highrtt | 125 kB/s | **11 kB/s** | 66 kB | 下限 2 MB |
| fast4g | 500 kB/s | **89 kB/s** | 534 kB | 下限 2 MB |
| fatpipe-highrtt | 2.5 MB/s | 276〜374 kB/s | 1.7〜2.2 MB | 下限 2 MB 前後で揺れる |
| normal | 無制限 | 10.7〜24.3 MB/s | 上限超え | 上限 20 MB |

**絞った回線では推定が実効の 1/5 以下しか出ない。** 原因は `noteBandwidth()` が
**1 リクエストごとに `wireBytes / 所要時間` を測っている**ことで、11 本が並列に
走っていれば 1 本あたりは 1/11 の速度に見える。**測っているのは回線の容量ではなく
1 接続あたりの速度である。**

結果、`slow-highrtt` と `fast4g` では `bw * 6` が下限を下回り、**下限 2 MB が
そのまま予算になっている**。`fatpipe-highrtt` はちょうど下限の境目に乗っていて、
推定のばらつきで常駐点数が 68,622 と 124,550 の間を run ごとに跳ぶ。

さらに **`budget()` を評価するのは点群の起動時とカメラ移動時だけ**である。
カメラを動かさなければ、**推定が最も当てにならない起動直後の値が最後まで残る。**

#### 予算を変えると何が起きるか（`fast4g`、12 秒窓）

| 予算 | PLATEAU | 建物 | 常駐点数 |
|---|---:|---:|---:|
| 0.25 MB | 10,970 ms | **22/22** | 12,612 |
| 0.5 MB | 10,918 ms | **22/22** | 12,612 |
| 1 MB | — | 20/22 | **68,622** |
| 2 MB（現行の実効値） | — | 20/22 | 56,448 |
| 点群 OFF | 9,852 ms | 22/22 | 0 |

**点群は確かに PLATEAU を押しのけている**（9.9 秒 -> 12.6 秒）。そして
**2 MB は 1 MB に劣る** — 建物は同じ 20/22 で、常駐点数だけが少ない。
12 秒の窓の中で取り切れない量を要求しているぶん、届いた点が減っている。

#### それでも定数だけ動かすのはやめた

下限を 1 MB に下げると `fast4g` は良くなるが、境目に乗っている
`fatpipe-highrtt` が 124,550 点から 68,622 点に落ちる。係数を `bw * 12` にしても
`fatpipe-highrtt` は戻らなかった（起動時の推定が低いまま固定されるため）。

**推定が 1/5 しか出ていない状態で係数を調整しても、合わせているのは
誤差のほうである。** 順番は (1) 集計を 1 接続あたりから回線全体に直す
(2) 推定が落ち着いたら予算を評価し直す (3) そのうえで係数を決める、になる。
`docs/TODO.md` に書いた。

#### `slow-highrtt` では関係ない

1 Mbps では**点群は 12 秒の窓で 1 ノードも要求していない**（`wantedPoints` 0）。
予算を 0.25 MB にしても 2 MB にしても `@10s` は 1,031,593 B で変わらない。
