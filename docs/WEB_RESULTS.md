...
### 5.1 点群のキャンセルは**機構が壊れていた** **[実測・2026-08-23]**

「点群ノードが AOI 全体を覆っていて `wanted` から外れないため」と書いていたが、
実点群（歩いた帯だけ、3.17 ha）に差し替えれば作れるはずだった。
シナリオを作り直して実配信で追い込んだところ、**理由は 2 つとも実装側にあった。**

まず地形側は正常に働いている。被覆外へ絶対座標で跳ぶシナリオで
**terrainFine が 16〜22 件キャンセル、無駄バイト 0**。

点群側は 0 件のまま。原因を 2 つ見つけた。

#### 原因 1: epoch のカウンタが別物だった **[修正済み]**

`Scheduler.stale()` は `task.epoch < scheduler.epoch` で古さを判定する。
ところが `PointCloudController` は**自分の `this.epoch`** を渡していた。
`main.ts` はカメラ移動のたびに `scheduler.setEpoch(+1)` してから
`controller.update()` を呼ぶので、コントローラ側は常に 1 進んでおり、
**`stale()` が恒常的に false**。点群の要求は原理的に一度もキャンセルされない。

`epoch: this.o.scheduler.currentEpoch` に直した。
ただしこれだけでは発火しなかった（原因 2 が残っていたため）。

#### 原因 2: **LOD がカメラを見ていない** **[未修正・高]**

直後に測ると、`wantedPoints` が **zoom 18.4 と zoom 12.5 で完全に同一（587,481）**、
被覆外へ 400 m パンしても同一だった。`stillNeeded` は
「選ばれているノードが 1 つでも残っているか」なので、**常に真**になる。

`main.ts` の `viewState()` がこうなっている:

```ts
const alt = map.transform?.cameraToCenterDistance ?? 1000
return { eye: [0, 0, Math.max(alt / 8, 60)], viewportHeight, fovY }
```

- `eye` の水平成分が **常に (0, 0)** = AOI 中心の真上に固定。
  カメラの中心・方位・傾きを一切見ていない
- 高度に使っている `cameraToCenterDistance` は
  **キャンバス高さと fov だけで決まり、ズームに依存しない**

つまり LOD は毎回同じ視点で評価しており、**選ばれるノードはカメラを
どう動かしても変わらない**。予算（60 万点）を埋めるだけの静的な選択になっている。

これはキャンセルが出ない理由であると同時に、**LOD が本来の仕事をしていない**
ということでもある。「近くは細かく、遠くは粗く」が効いていない。

コード上のコメントは
「ローカル原点は AOI 中心なので、視点は中心の真上として扱えば十分（AOI は 1 km 四方）」
と、**意図的な簡略化として書かれている**。1 km 四方なら水平位置の影響は小さい、
というのは妥当な近似だが、**ズームに追従しない点は近似ではなく誤り**である。

#### 検証に必要だった道具

- `perf/run.mjs --scenario=cancel` を作り直した。被覆外へ**絶対座標で**跳ぶ
  （相対移動だと被覆から出たかが積算に左右されて確かめられない）。
  `pc_index_loaded` を待ってから振る（待たないと点群の要求が 1 本も出ていない）。
  `--dwell` と `--maxbytes` を足した
- `?maxbytes=` を viewer に足した。**絞った回線では LOD のバイト予算
  （`帯域 × 6`）が `pcFine` の発行そのものを止めてしまい、
  「キャンセルすべき飛行中の要求」が作れない。** 予算とキャンセル機構は
  この点で相反していて、予算を緩めないと経路の検証ができない
...
**ただし、この 2 つはどちらも「タイルの値が正しいか」しか見ていない。**
「その値が画面のどこに出ているか」は誰も見ていなかった（§7.1）。

### 7.1 タイルの中で南北がひっくり返っていた **[実測・修正済み]**

**症状**: 地面と浸水が建物と噛み合わない。川が途切れて別の場所に出る。

**原因**: 焼いたタイルは **1 行目が北**（`scripts/80` が north-up で焼く）。luma.gl は
`UNPACK_FLIP_Y_WEBGL` を立てないので、テクスチャ座標 `t=0` が北辺になる。一方
`FloodMeshLayer` の uv は `mix(bounds.xy, bounds.zw, aUv)` で使う都合上 **`v=0` が南**。
反転せずに `texture(tex, uv)` で引いていたので、**タイル 1 枚の中で南北が反転**していた。
板（`BitmapLayer`）だった時期は deck.gl が `texCoords` に `1-v` を入れていたので
起きず、メッシュ化（`docs/WEB_DESIGN.md` §2.2.1）で持ち込んだ。

ずれ量は**タイルの高さそのもの**（z15 で 1.2 km、z18 で 150 m）。

**なぜ半セルずれの調査で見つからなかったか**: `perf/tileshift.mjs` は
「粗タイルから細タイルに切り替わるときに平行移動するか」を見る道具で、
**全ズームで同じ向きに間違っている**誤りには反応しない。半セル（0.24〜1.95 m）は
実際にあった別の誤りで、それを直しても向きは直らない。

あわせて**法線の y 成分の符号も逆**だった。面 `z = f(x, y)` の法線は `(-fx, -fy, 1)` で、
x と y は同じ向きでなければならない。反転したサンプルと打ち消し合って**陰影だけは
それらしく見えていた**ので、絵を見ても気づけなかった。

**道具**: `web/perf/tileorient.mjs`。真上から撮った画面の各画素を `map.unproject` で
lng/lat に直し、**焼いた PNG の同じ座標の (標高, h_conn)** と突き合わせる。

> **視差の補正が要る。** `map.unproject` は **z=0 平面**との交点を返すが、地表は
> `geoid 36.955 m + 標高` の高さに描かれている。zoom 17 のカメラ高度は 510 m なので
> **7.2 %（画面端で 41 px）外側**に写る。これを入れずに照合すると、汀線の食い違いが
> 「向きのずれ」なのか視差なのか分けられない（補正前は修正後でも 88 % 台に留まった）。

**測定**（H=1.0 m / 真上 / zoom 17 = z18 タイル / 4 画素ごと 約 39,000 点）:

| | 修正前 | 修正後（3 回） |
|---|---|---|
| 画面と焼いたタイルの一致率 | **78.7 %** | **97.3〜97.6 %** |
| 標高が H±0.25 m の画素と汀線 1 セルの帯を除く | 89.9 % | **99.6〜100.0 %** |

- zoom 15（z16 タイル）/ H=0.93 m でも 97.7 % / 99.9 %。
- 参照を ±2 セル（1 セル = 0.49 m）動かす掃引でも **ずらさない時がほぼ最良**。
  最良は半セル（0.24 m）ずらした点で、利得は +0.1〜0.2 ポイント。視差補正自体の
  誤差の範囲。修正前の掃引はどこも 88 % 前後の平坦で、
  **平行移動では説明できない**形だった。
- 同じビルドでも照合点数が数十点動く（画面のどのタイルが揃った時点で撮るかで変わる）。
  **0.3 ポイント程度の差は読まない。**

**残る 2.7 % はどこか**: 標高が H の前後にある画素（照合点の 35 %）に集中する。
メッシュは 128 分割でテクスチャは 256 なので、水深は**2 分の 1 に間引いた標高**から
作られる。吉原は起伏 0〜3 m で H=1.0 m がその真ん中なので、ここが動く。

**測って入れなかったもの**: 水深と地面色を**画素ごとにセルの標高で**引く版
（テクスチャは同じなので転送量は増えない）。97.5 % / 99.6 % で、採用版の範囲の中に入る。
**改善は確認できなかったので入れていない。**
