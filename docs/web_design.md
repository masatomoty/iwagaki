...
| | 塗り | 輪郭 |
|---|---|---|
| 建物 | 乾 = 冷たい灰 / 浸水 = 青 / **判定変化 = 赤** | 判定変化のとき黄 |
| 道路 | 乾 = 暖色の淡色 / 浸水 = **通行支障クラス**（淡クリーム → 琥珀 → 橙 → 赤） | 同じ（判定変化 = 黄） |

**道路の塗りは通行支障だけを表す。** 判定が変わったことは輪郭（黄）が担う。
塗りに 2 つの意味（判定変化の赤 / 通行支障の赤）を載せると、
どちらを見ているのかを画面から決められない。
閾値は解析側が持っている `config.py` の `ROAD_DEPTH_CLASSES = (0.1, 0.3, 0.5)` が
catalog 経由で来るので、ブラウザ側に数字は書かない。

浸かった道路の色は**水と混ざらない暖色〜赤の一本道**にしてある。
最初は最浅クラスを水色にしていたが、水面（水色）の下に置くと消えた [実測]。
