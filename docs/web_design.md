...
### 線路は 1 px の線では読めなかった **[実測]**（2026-08-25）
...
最初は点群の被覆輪郭と同じ `LineSegments` で出した。**読めなかった** [実測] —
WebGL の線幅は 1 px 固定で、しかも道路が**一律のほぼ白**なので、
無彩色の 1 px 線は道路に紛れる。**道路と同じくジオメトリに幅を持たせる**
（ワールド座標のリボン、幅 9 m = 複線の路盤相当）。

| 決めごと | なぜ |
|---|---|
| 幅はワールド座標の 9 m | 1 px 固定の GL 線では道路（ほぼ白）に紛れる。ズームに応じて太さが変わる方が地図として素直でもある |
| 色は**黒白の交互**（刻み 18 m） | 画面の色は 1 つの予算で、地面＝灰 / 建物＝灰・黄・赤 / 水＝青 / 道路＝ほぼ白 / 窪地＝薄い水色 が埋まっている。**新しい色相を割ると必ずどこかとぶつかる**ので無彩色にし、道路との区別は色ではなく**刻みがあること**に持たせた（地図記号の慣例） |
| 刻みはフラグメントで作る | 累積距離を varying で渡して縞を計算する。頂点属性で持つと頂点間で補間されて縞の境目がぼける |
| 標高は**頂点に焼いてある** | `scripts/12` が 0.5m DEM から引く。地形タイルを待たずに描け、鉛直強調は uniform 1 個で済む（`setExaggeration`） |
| `depthTest` は切る | 参照線なので、山や建物の裏に隠れると用を成さない（被覆輪郭と同じ扱い） |
| 線路が無い範囲では**選択肢ごと出さない** | 吉原 100 ha には線路が掛からない。catalog に `railway` が無ければ `重ねる` にも凡例にも出さない（`layersOf()`。点群と同じ規則） |

主題は浸水であって鉄道ではないので、**広い初期表示では控えめ**（幅 9 m は
4 km 幅の画面で 2〜3 px）、寄ると地図記号として読める、という強さに置いた。
