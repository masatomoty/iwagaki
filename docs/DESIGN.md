...
5. 高潮水位は**指定ハザードではなくパラメータ**。ただし
   **舞鶴は高潮浸水想定区域（水防法第14条の3）に指定されている**
   （`docs/DATA.md` §4。以前ここに「指定は存在しない」と書いていたのは誤り）。
   京都府の想定は 910 hPa（室戸台風規模）で、基準潮位 T.P.+0.69 m から
   潮位偏差を足す。**堤防等は決壊するものとして扱っている。**
   本リポジトリのモデルは施設を一切持たないので、決壊側の想定に近い。
